...
orc_walk_anim = [orc_sprite_sheet.subsurface(pygame.Rect(x * 100, 100, 100, 100)) for x in range(6)]
orc_attack_anim = [orc_sprite_sheet.subsurface(pygame.Rect(x * 100, 2 * 100, 100, 100)) for x in range(6)]
orc_frames = orc_walk_anim
...
class Agent:
    def __init__(self):
        ...
        self.current_state = AgentState.PATRO_STATE
        self.current_anim = orc_walk_anim

    def update(self, target):
        a = pygame.Vector2(0, 0)
        if self.current_state == AgentState.PATRO_STATE:
    ...
    def draw(self, screen):
        ...
        if self.current_state == PATRO_STATE:
            pygame.draw.circle(screen, (0, 0, 255), self.position, 10)
        elif self.current_state == CHASE_STATE:
            pygame.draw.circle(screen, (225, 255, 0), self.position, 10)
        elif self.current_state == ATK_STATE:
            pygame.draw.circle(screen, (225, 0, 0), self.position, 10)
